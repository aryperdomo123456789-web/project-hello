        ...
        {columns.map(column => (
          <div key={column.id} className="w-80 flex flex-col h-full group">
            ...
            <div className="flex-1 space-y-3 overflow-y-auto pr-2 custom-scrollbar">
              {contacts.filter(c => c.stage === column.id).map(contact => (
                <div 
                  key={contact.id} 
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {contact.tags.map(tag => (
                      <Badge key={tag} variant="secondary" className="px-2 py-0.5 text-[9px] uppercase font-bold bg-slate-100 text-slate-500 border-none">
                        {tag}
                      </Badge>
                    ))}
                  </div>

...
import { Eye, Megaphone, Play, RefreshCw } from "lucide-react";
...
export function CampaignsView() {
  ...
  const load = useCallback(async () => {
    try {
      const [campaignRows, contactRows] = await Promise.all([listCampaigns(), listContacts()]);
  ...
  const canCreate =
    name.trim().length >= 2 && template.trim().length > 0 && selectedCount > 0 && !loading;
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setName("");
      setStatus("Campanha criada em modo sandbox; nenhum WhatsApp foi acionado.");
    } catch (error) {
      setStatus(error instanceof Error ? error.message : "Não foi possível criar a campanha");
      captureDiagnostic(error, {
        source: "async",
        component: "CampaignsView",
        payload: { operation: "create_campaign", selectedCount },
        recoverable: true,
      });
    } finally {
      setLoading(false);
        ...
        <header className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-1 max-w-2xl text-sm text-slate-500">
              Crie e simule campanhas com opt-out, frequência e limite por dia. O modo atual é
              sandbox: nenhum provedor externo é acionado.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]">
          <section className="rounded-2xl border bg-white p-6 shadow-sm">
            ...
            <div className="mt-5 space-y-4">
              ...
              >
                <Play className="h-4 w-4" /> Criar campanha sandbox
              </button>
            </div>
          </section>
          ...
          <section className="rounded-2xl border bg-white p-6 shadow-sm">
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="mt-1 text-xs text-slate-500">
                          {campaign.status} · limite {campaign.dailyLimit}/dia · frequência{" "}
                          {campaign.frequencyHours}h
                        </p>
                      </div>
                      <button
                        type="button"
            ...
            {preview && (
              <div className="mt-5 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900">
                <p className="font-bold">Prévia sandbox</p>
                <p className="mt-2">
                ...
                <p className="mt-2 text-xs">Modo: {preview.mode}. Nenhuma mensagem foi enviada.</p>
              </div>
            )}
          </section>
        </div>
